...
本專案採用 **「情境感知（Context-Aware）」的混合 Session 架構**。根據使用場景（私人私聊 vs 群組聊天）智慧切換「多輪連續對話」與「單次無狀態問答」，兼顧深度對話連續性與群組對話摘要的乾淨度：

### 📊 各場景記憶模式對照表

| 使用場景 | 記憶機制 | Firebase 儲存路徑 | 設計理念與運作流程 |
|---|---|---|---|
| **私人 1 對 1 聊天** | 🧠 **多輪連續記憶 (Full Session)** | `users/{user_id}/message_events` | **如 ChatGPT 連續對話**：自動取出用戶歷史上下文送入 LLM 進行推論，回覆後寫回 Firebase。輸入 `!清空` 或 `!clean` 可重設記憶。 |
| **群組一般聊天記錄** | 📝 **長篇歷史累積 (Transcript)** | `groups/{group_id}/message_events` | **供 `!摘要` 總結使用**：平時默默記錄群組各成員的對話，當下達 `!摘要` 或 `!wiki summary` 時，取出全量記錄進行結構化整理。 |
| **群組 `@機器人` 提問** | ⚡ **單次無記憶 (Stateless Q&A)** | *主動剔除不存檔* | **防污染與省 Token**：群組話題繁雜，若每次 @ 都帶入數百條聊天紀錄會造成回答失焦與 Token 浪費。因此提問後立即以 `messages.pop()` 移除，不污染後續摘要。 |
| **工具指令 (`!search`/`!畫圖`/`!help`)** | 🚫 **單次無記憶 (Tool Mode)** | *主動跳過存檔* | **保持對話純淨**：各類系統指令與臨時工具查詢不寫入歷史，確保對話紀錄聚焦於核心內容。 |

### 🔍 核心架構與可靠性設計

```
[ 用戶訊息傳入 ] ──▶ [ 分散式原子鎖 acquire_message_lock (防止重試重複處理) ]
                         │
        ┌────────────────┴────────────────┐
        ▼                                 ▼
   【私人對話】                      【群組對話】
        │                                 │
   讀取用戶專屬 Session              讀取群組 Transcript
        │                                 │
  帶入歷史多輪 Context                    ├── 包含指令 (!摘要/!wiki) ➔ 全量總結
        │                                 └── 被 @ 提及 ➔ 單次問答 (不寫入歷史)
        ▼                                 │
   寫入 Firebase                     寫入群組 Transcript
```

1. **持久化與隔離性**：
   - 所有 Session 與 Transcript 皆保存在 **Firebase Realtime Database (RTDB)**。
   - 私人對話（`users/{user_id}`）與群組對話（`groups/{group_id}`）路徑完全物理隔離，不同群組與不同使用者彼此互不干擾。
2. **分散式訊息原子鎖 (Idempotent Lock)**：
   - 透過 `services/firebase.py` 內建之 `acquire_message_lock`，在處理任何事件前先以 ETag 條件寫入鎖定 LINE `message.id`，徹底防止 LINE 重試連線或多容器部署時發生重複回覆問題。
3. **動態記憶重設**：
   - 隨時傳送 `!清空`、`！清空` 或 `!clean`，即可完全抹除該私聊或群組的 Session 紀錄，從零開始新話題。
